...
void accelerator(
    ...
    for (int epoch = 0; epoch < NUM_ITERATIONS; ++epoch) {
        ...
        for (int iteration = 0; iteration < input.size(); ++iteration) {
            ...
            std::vector<std::vector<double>> result_l1 = forwardPropagation(input_ref, weights_l1, biases_l1, ACTIVATION_HIDDEN);
            ...
            std::vector<std::vector<double>> d_l1 = backProp(weights_l2, d_l2, input_ref, weights_l1, biases_l1, 0);
            ...
            updateWeightBias(weights_l1, biases_l1, input_ref, d_l1, learning_rate);
