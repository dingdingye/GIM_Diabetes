...
void accelerator(
    const std::array<std::array<std::array<double, 1>, IN_SIZE>, PARAM_DATA_SIZE>& input,
    const std::array<std::array<double, OUT_SIZE>, PARAM_DATA_SIZE>& y_true,
    ...
    for (int epoch = 0; epoch < NUM_ITERATIONS; ++epoch) {
        ...
        if (!test) {
            for (int iteration = 0; iteration < input.size(); ++iteration) {
                // printf("======================\n");
                // printf("iteration %d \n", iteration);
                std::array<std::array<double, 1>, IN_SIZE> input_ref;
                for (int i = 0; i < IN_SIZE; ++i) {
                    input_ref[i][0] = static_cast<double>(input[iteration][i][0]);
                }
            ...
            accuracy = double((float)correct / input.size());
        }
    }
    int correct = 0;
    if (test){
