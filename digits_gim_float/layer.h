    ...
    const std::array<std::array<double, COLS_B>, COLS_A>& B)
{
    ...
    std::array<std::array<double, COLS_B>, ROWS_A> C = {}; 
...
std::array<std::array<double, 1>, OUTPUT_SIZE> forwardPropagation(
    std::array<std::array<double, 1>, INPUT_SIZE>& input,
    std::array<std::array<double, INPUT_SIZE>, OUTPUT_SIZE>& weights,
    ...
    int activation)
{
    ...
    std::array<std::array<double, 1>, OUTPUT_SIZE> mid = matmul(weights, input); // weights * input

    std::array<std::array<double, 1>, OUTPUT_SIZE> net{};  // initialize net with zeros to be same size as mid
    ...
    std::array<std::array<double, 1>, OUTPUT_SIZE> output{}; // initialize output with zeros to be same size as net
...
std::array<std::array<double, 1>, INPUT_SIZE> backProp(
        const std::array<std::array<double, INPUT_SIZE>, PLUS_1_SIZE>& w_l_plus1,
        const std::array<std::array<double, 1>, PLUS_1_SIZE>& d_l_plus1,
        const std::array<std::array<double, 1>, OUTPUT_SIZE>& input,
        const std::array<std::array<double, OUTPUT_SIZE>, INPUT_SIZE>& weights,
        const std::array<double, INPUT_SIZE>& biases,
        int activation)

{
    
    std::array<std::array<double, 1>, INPUT_SIZE> d_l{}; // initialize d_l with zeros
    std::array<std::array<double, PLUS_1_SIZE>, INPUT_SIZE> w_l_plus1_T = transpose<PLUS_1_SIZE, INPUT_SIZE>(w_l_plus1); // transpose w_l_plus1
    std::array<std::array<double, 1>, INPUT_SIZE> temp = matmul(w_l_plus1_T, d_l_plus1); // w_l_plus1_T * d_l_plus1
    
    // Getting the net term for this layer (net = matmul(weights, input) + biases)
    std::array<std::array<double, 1>, INPUT_SIZE> mid = matmul<INPUT_SIZE, OUTPUT_SIZE, 1>(weights, input);
    std::array<std::array<double, 1>, INPUT_SIZE> net{}; // Net should be the same size as biases
    ...
    if (activation == 0) {
        // std::array<std::array<double, 1>, OUTPUT_SIZE> d_activation = derivative_relu(net);
        std::array<std::array<double, 1>, INPUT_SIZE> d_activation = derivative_relu<INPUT_SIZE>(net);
...
void updateWeightBias (
    std::array<std::array<double, INPUT_SIZE>, OUTPUT_SIZE>& weights,
    std::array<double, OUTPUT_SIZE>& biases,
    const std::array<std::array<double, 1>, INPUT_SIZE>& input,
    const std::array<std::array<double, 1>, OUTPUT_SIZE>& d_l,
    double learning_rate)
{
    std::array<std::array<double, INPUT_SIZE>, 1> input_T = transpose(input);                      // transpose input
    std::array<std::array<double, INPUT_SIZE>, OUTPUT_SIZE> update_temp_mat = matmul(d_l, input_T);   // compute update matrix
